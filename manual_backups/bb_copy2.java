...
public class BranchAndBound{
    ...
    public void lineKthMax(int iline, int jcolumn, int K) {
        ...
        sortLines(iline, jcolumn);
        matrix.permutRows(iline, iline + K);
        reorganizeColumns(iline, jcolumn);
    ...
    public void createChildren(Node parent, int currentj) {
        ...
        if (parent.nbWhites == 1){
            parent.noChildren=true;
        }else{
        sortLines(parent.depth + 1, Math.max(currentj,0));
        reorganizeColumns(Math.min(parent.depth + 1, matrix.getNbRows()-1) , Math.max(currentj,0));
